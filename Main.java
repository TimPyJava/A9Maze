...
This program will make a maze and solve it. There is no input from the user. There is an object that will be used to make the maze. There is also a method that will solve the maze. If the maze is solvable, the cheese will be found. If not, the chees will not be found.
*/
...
class Main {
  ...
  public static boolean find_path(Maze my_maze,int x, int y) {
    ...
    //Check if we have already been to this cell via other recursive calls.  You'll know we've been here if the cell has a + in it.  If cell x,y of theMaze is x, return false.  Don't return true otherwise.
    ...
    if(find_path(my_maze, x-1, y)==true){
      return true;
    }
    ...
    if(find_path(my_maze, x, y+1)==true){
      return true;
    }
